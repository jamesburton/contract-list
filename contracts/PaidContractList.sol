...
contract PaidContractList is Ownable {
    ...
    event AddedAddress(address name, uint addressCount);
    event RemovedAddress(address name, uint addressCount);
    ...
    function remove(address _address) public payable {
        if(msg.value < price)
            revert InsufficientPayment(msg.value, price);
